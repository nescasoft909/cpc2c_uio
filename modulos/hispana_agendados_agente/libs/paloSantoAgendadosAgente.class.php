...
class paloSantoAgendadosAgente{
    ...
    function getNumReporteAgendados($filter_field, $filter_value, $user,$aditional_key,$aditional_value,$calltype)
    {
        $where    = "  ";
        $whereCalltype ="";
        $arrParam = null;
	    ...
	    $whereCalltype = " AND (`f`.`id`=$calltype)";            
        }
    ...
    where
        ((`a`.`ci` = `b`.`ci`)
            ...
            and (`e`.`id` in (`b`.`id_campania`,`b`.`id_campania_consolidada`))
            and (`d`.`id_calltype` = `f`.`id`)
			and (`d`.`id_campania` = `e`.`id`)
			and (`b`.`agente_agendado` = '$user')
            and (`e`.`tipo` <> _utf8'RECARGABLE') $where $whereCalltype ) 
    union select distinct
        ...
        `a`.`ci` AS `ci`,
        `e`.`nombre` AS `campania`,
        `b`.`agente_status` AS `agente`,
        _utf8'' AS `telefono`,
        _utf8'' AS `contactabilidad`,
        _utf8'' AS `mejor_calltype`,
        `b`.`id_gestion_mejor_calltype` AS `id_gestion_mejor_calltype`,
        _utf8'' AS `observacion`,
        `b`.`fecha_agendamiento` AS `fecha_agendamiento`,
        `b`.`agente_agendado` AS `agente_agendado`,
        `a`.`origen` AS `origen`,
        0 AS `peso`,
    ...
    from
        ...
        join `campania_cliente` `b`)
        join `campania` `e`)
        join `campania_agente` `f`)
    where
        ((`a`.`ci` = `b`.`ci`)
            and isnull(`b`.`id_gestion_mejor_calltype`)
            and (`b`.`id_campania` = `e`.`id`)
            and (`e`.`tipo` <> _utf8'RECARGABLE')
            and (`b`.`id_campania` = `f`.`id_campania`)
            and (`b`.`agente_agendado` = convert( `f`.`id_agente` using utf8))
			and (`b`.`agente_agendado` = '$user')
            and (`a`.`origen` <> _utf8'base') $where ) 
    union select distinct
        `e`.`id` AS `id_campania`,
    ...
    function getReporteAgendados($limit, $offset, $filter_field, $filter_value, $user,$aditional_key,$aditional_value,$calltype)
    {
        $where    = "  ";
        $whereCalltype = "";
        $arrParam = null;
...
select distinct UCASE(tipo) from cliente_gestion_adicionales where tipo not in ('telefono','direccion')
    ...
    function getCalltypes(){
        $query = "select id,descripcion from calltype where status='A'";
	...
	$result=$this->_DB->fetchTable($query, true);
        ...
        $arrCalltypes=array(""=>"TODOS");

	foreach($result as $k => $calltypes){
	    $arrCalltypes[$calltypes['id']] = $calltypes['descripcion'];
	}
