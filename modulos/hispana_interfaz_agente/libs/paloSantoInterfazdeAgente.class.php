...
*/
    ...
    {
	$query = " select count(*) as cont from (select distinct id_campania_cliente as cont from gestion_campania where date(fecha)=date(now()) and agente='$user' and id_campania_cliente>0) x";
	$result = $this->_DB->getFirstRowQuery($query,true);
        $resultado=$result['cont'];
        $query = " select count(*) as cont from (select distinct id_campania_recargable_cliente from gestion_campania where date(fecha)=date(now()) and agente='$user' and id_campania_recargable_cliente>0) x";
	$result = $this->_DB->getFirstRowQuery($query,true);
        $resultado+=$result['cont'];
	return $resultado;
    }
