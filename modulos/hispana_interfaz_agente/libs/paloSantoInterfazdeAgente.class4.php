    ...
    */
	...
	$query = "SELECT
		  ...
		  AND c.ci=d.ci 
                  ...
                  SELECT
                  c.id as 'id_campania_cliente',
                  c.id as 'id_campania_consolidada',
                  b.id as 'id_campania',
                  b.tipo as 'tipo_campania',
                  b.nombre as 'campania', 
                  d.ci, 
                  ...
                  c.fecha_agendamiento,
                  c.fecha_agendamiento-now() as 'segundos_faltantes'
                  FROM campania_agente as a, campania as b, campania_recargable_cliente as c, cliente_gestion as d  
                  WHERE 
                  b.fecha_inicio<=date(now()) 
                  AND b.fecha_fin>=date(now()) 
                  AND b.status='A'
	...
	if(!isset($result['ci'])){
	    ...
	    $this->_log->output("paloSantoInterfazdeAgente->obtenerCliente (3) \$result " . print_r($result,true));
            ...
            if(isset($result['ci'])){
                ...
                $query = "SELECT tipo as descripcion,adicional
                ...
                //obtener telefonos
                $query = "SELECT tipo as descripcion,adicional as telefono
                          FROM cliente_gestion_adicionales 
                          WHERE id_cliente = '$result[id_cliente]'
                          AND tipo = 'telefono'
                          AND status='A'";
                ...
                //obtener direcciones
                $query = "SELECT tipo as descripcion,adicional as direccion
                          FROM cliente_gestion_adicionales 
                          WHERE id_cliente = '$result[id_cliente]'
                          AND tipo = 'direccion'
                          AND status='A'";
    ...
    function guardarGestion($DATA,$agente)
    {
        $query = "INSERT INTO gestion_campania (id_campania_cliente,calltype,timestamp,telefono,observacion,agente,id_campania_recargable_cliente,fecha)
		  VALUES ('" . $DATA['id_campania_cliente'] . "','"
			     ...
			     . $DATA['observacion'] . "','"
			     . $agente .  "',"
                             . $_SESSION["id_campania_cliente_recargable"] . ",now())";
	...
	// Inicio: Actualizo id_gestion_mejor_calltype en campania_cliente
        if($_SESSION["id_campania_cliente_recargable"]){
            $query = "SELECT a.id, calltype, b.peso 
                      ...
                      ORDER BY b.peso DESC,fecha DESC LIMIT 1";
            $result = $this->_DB->getFirstRowQuery($query, true);
          ...
          $query = "SELECT a.id, calltype, b.peso 
                    FROM gestion_campania AS a, calltype as b,calltype_campania as c
                    WHERE 
