...
function _moduleContent(&$smarty, $module_name)
{
    ...
    switch($action){
        default:
            $content = reportEmisionPago($smarty, $module_name, $local_templates_dir, $pDB, $arrConf);
            break;
    }
    return $content;
}

function reportEmisionPago($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf)
{
    ...
    $oGrid->setNameFile_Export(_tr("Emision con pago"));
    ...
    $arrColumns = array(_tr("Campaña"),_tr("Origen"),_tr("Fecha Gestion"),_tr("Cliente"),_tr("Teléfono"),/*_tr("Agente"),*/_tr("Calltype"),_tr("Observación"),_tr("Acción"));
    //Datos Adicionales
    ...
    $total   = $pReportedeCalltypes->getNumReporteAgendados($filter_field, $filter_value, $_SESSION['elastix_user'],$aditional_key,$aditional_value);
    $arrData = null;
    ...
    $arrResult =$pReportedeCalltypes->getReporteAgendados($limit, $offset, $filter_field, $filter_value, $_SESSION['elastix_user'],$aditional_key,$aditional_value);

    if(is_array($arrResult) && $total>0){
        foreach($arrResult as $key => $value){	    
	    $arrTmp[0] = $value['campania'];
	    $arrTmp[1] = $value['origen'];
	    $arrTmp[2] = $value['fecha'];
