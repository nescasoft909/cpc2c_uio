...
function reportReportedeCalltypes($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf)
{
    ...
    $oGrid->setNameFile_Export(_tr("Reporte de Campañas Recargables"));
    ...
    // Columnas base
    $arrColumns = array(_tr("Fecha"),_tr("Hora"),_tr("Campaña"),_tr("Cliente"),_tr("CI"),_tr("Teléfono"),_tr("Agente"),_tr("Contactabilidad"),_tr("Calltype"),
                        _tr("Provincia"),_tr("Ciudad"),_tr("Nacimiento"),_tr("Correo Personal"),_tr("Correo Trabajo"),_tr("Estado Civil"),_tr("Formulario"),);

    $oGrid->setColumns($arrColumns);

    $total   = $pReportedeCalltypes->getNumReportedeCalltypes($filter_field, $filter_value, $filter_field_adicional,$filter_value_adicional);
    ...
    $arrResult =$pReportedeCalltypes->getReportedeCalltypes($limit, $offset, $filter_field, $filter_value, $filter_field_adicional,$filter_value_adicional);
    ...
    if(is_array($arrResult) && $total>0){
	    ...
	    $arrTmp[8] = $value['calltype'];
            $arrTmp[9] = $value['provincia'];
            $arrTmp[10] = $value['ciudad'];
            $arrTmp[11] = $value['nacimiento'];
            $arrTmp[12] = $value['correo_personal'];
            $arrTmp[13] = $value['correo_trabajo'];
            $arrTmp[14] = $value['estado_civil'];
	    // $arrTmp[8] = $value['formulario'];

	    // Así lo requiere RUBENING
	    $arrTmp[15] = "<a href=modules/$module_name/gestion_info.php?id_gestion=$value[id_gestion] target=\"_blank\" onClick=\"window.open(this.href, this.target, 'width=600,height=400'); return false;\">Ver gestión</a>".
                    "<br>".
