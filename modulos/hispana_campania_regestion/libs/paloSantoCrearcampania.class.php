...
class paloSantoCrearcampaña{
    ...
    function getBasesCampania($id_campania)
    {
        $query = "SELECT a.id, a.nombre 
		  ...
		  FROM base as a, campania_recarga as b
		  WHERE 
		  b.status = 'A'
		  AND a.id=b.id_base
		  AND b.id_campania=$id_campania";
    ...
    function getCalltypes($id_campania, $clase_calltype)
    {
	$query = "SELECT a.id, a.descripcion 
		  FROM calltype as a, calltype_campania as b 
		  WHERE a.id=b.id_calltype 
		  AND a.clase='$clase_calltype' 
		  AND b.id_campania=$id_campania 
		  AND a.status='A'";
    ...
    function guardarCampaña($DATA) 
    {
	$query = "INSERT into campania (nombre,fecha_inicio,fecha_fin,id_form,script,status) 
		  VALUES ('".$DATA['nombre']."','"
			    .$DATA['fecha_inicio']."','"
			    .$DATA['fecha_fin']."',"
			    .$DATA['id_form'].",'"
	...
	foreach($result as $cliente){
	    $query = "INSERT INTO campania_cliente (id_campania, ci, prioridad) 
		      VALUES  (".$idCampaign.",'"
				.$cliente['ci']."',$cliente[prioridad])";
