...
left join cliente_gestion_adicionales cga on (cga.id_cliente=crc.id_cliente)
	...
	foreach($result_ci as $arrCi){
	    $query = "select c.nombre as campania,
                             b.nombre as base,
                             b.id as id_base,
                             ct.descripcion as calltype, 
                             ct.clase as contactabilidad,
                             ...
                             gc.agente as agente,
                             gc.telefono,
                             concat(cg.nombre,' ',cg.apellido) as ' cliente',
                             gc.id as id_gestion,
                             cc.peso,
                             gc.calltype as id_caltype,
                             cg.provincia,
                             cg.ciudad,
                             cg.nacimiento,
                             cg.correo_personal,
                      ...
                      where gc.id=(select max(y.id) from gestion_campania y where y.id_campania_recargable_cliente=$arrCi[id])
                      and crc.id=gc.id_campania_recargable_cliente
                      and c.id=crc.id_campania
                      and cc.id_campania=c.id
                      and cc.id_calltype=ct.id
                      and gc.calltype=ct.id
