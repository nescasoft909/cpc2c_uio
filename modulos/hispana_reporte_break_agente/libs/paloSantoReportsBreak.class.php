...
class paloSantoReportsBreak
{
     ...
     */
    function getReportesBreak($fecha_init,$fecha_end,$agente)
    {
...
SELECT break_agente.id_agente, break_agente.id_break, break.name AS nombre_break, 
    SUM(UNIX_TIMESTAMP(IFNULL(break_agente.datetime_end, NOW())) - UNIX_TIMESTAMP(break_agente.datetime_init)) AS duracion
FROM  break_agente, break 
WHERE break.id = break_agente.id_break 
AND break_agente.datetime_init >= ? AND break_agente.datetime_init <= ?
AND break_agente.id_agente='$agente'
GROUP BY break_agente.id_agente, break_agente.id_break
LEER_SUMARIO_BREAK;
