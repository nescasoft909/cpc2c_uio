...
class paloSantoDataForm
{
    ...
    function obtener_campos_formulario($id_formulario,$id_campo=NULL)
    {
        ...
        $query  = "
                    SELECT  fd.id, fd.etiqueta, fd.value, fd.tipo, fd.orden, fd.status 
                    FROM  form_field fd
    ...
    function cambiar_status_field($id_field,$status)
    {
         $query  = "UPDATE form_field set 
                    status = '$status'
                    WHERE id = $id_field";
    ...
    if(is_array($arr_campos)&& count($arr_campos)>0){
        foreach($arr_campos as $key => $field) {
            ...
            if($edit){
                ...
                if($field['status']=='A')
                    $nodoContenido .= " <td class='table_data'><a href='?menu=hispana_form_designer&action=editarCampo&status=I&id=".$_GET['id']."&id_field=".$field['id']."'>"._tr('Inactivar')."</a></td>\n";
                else
                    $nodoContenido .= " <td class='table_data'><a href='?menu=hispana_form_designer&action=editarCampo&status=A&id=".$_GET['id']."&id_field=".$field['id']."'>"._tr('Activar')."</a></td>\n";
            }
            $nodoContenido .= "</tr>\n";
        }
    }
...
function cancelar_formulario_ingreso($id_formulario)
{
    ...
    if(!$oDataForm->eliminar_formulario($id_formulario)){
        ...
        $respuesta->addScript("window.open('?menu=hispana_form_designer','_parent')");
    return $respuesta;
}
...
function guardar_formulario($id_formulario,$form_name,$form_description,$lugar)
{
    ...
    if($validar=='true'){
        ...
        if(!$se_creo){
            ...
            if($lugar=='edit')
                $respuesta->addScript("window.open('?menu=hispana_form_designer&action=view&id=$id_formulario','_parent')");
            }
    }
    else{
        $respuesta->addAssign("mb_title","innerHTML",_tr("Validation Error")); 
...
function desactivar_formulario($id_formulario)
{
    ...
    if($oDataForm->eliminado_logico_formulario($id_formulario)) header('?menu=hispana_form_designer');
